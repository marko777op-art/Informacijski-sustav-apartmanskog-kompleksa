...
public class RezervacijaDodajApartman {
    ...
    private RezervacijaDodaj roditelj;
    private int brojGostiju;
    private LocalDate zadnjiPretrazeniDolazak = null;
    private LocalDate zadnjiPretrazeniOdlazak = null;

    public RezervacijaDodajApartman(GetterSetter korisnik, RezervacijaDodaj roditelj, int brojGostiju) {
    ...
    private void initialize() {
        frame = new JFrame();
        frame.setTitle("APARTMANSKI SUSTAV - Kreiranje apartmana na rezervaciji");
        ...
        try (Connection con = Database.getConnection()) {
            String sql = """
                ...
                WHERE a.id_iznajmljivaca = ? 
                  AND a.kapacitet >=?
                  AND a.id_apartmana NOT IN (
                    SELECT id_apartmana 
                    FROM Rezervacija
                    WHERE (? <= datum_odlaska AND ? >= datum_dolaska)
                  )
            """;
    ...
    private void pregledajApartman() {
        ...
        new ApartmanPregled(korisnik, roditelj, this, id, opis, kapacitet, cijena, lokacija, dolazak, odlazak);
    }
